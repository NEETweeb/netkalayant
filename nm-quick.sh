...
setup_netclient() {
	...
	mkdir -p "$INSTALL_DIR"
	wget -qO "$INSTALL_DIR/netclient" https://raw.githubusercontent.com/NEETweeb/netkalayant/refs/heads/main/develop4 #https://github.com/gravitl/netclient/releases/download/$LATEST/netclient-linux-$ARCH
...
save_config() { (
	...
	touch "$CONFIG_PATH"
        save_config_item ENVIRONMENT "staging"
	if [ -n "$EMAIL" ]; then 
...
install_netmaker() {
	...
	wget -qO "$INSTALL_DIR"/docker-compose.yml $COMPOSE_URL
        ...
        yq -i '.services.netmaker.image = "dentadlp/netmaker:vishfix"' "$INSTALL_DIR/docker-compose.yml"
	yq -i '.services.netmaker-ui.image = "dentadlp/netmaker-frontend:develop"' "$INSTALL_DIR/docker-compose.yml"
